...
pub fn ug_mutator_release<VM: VMBinding>(mutator: &mut Mutator<VM>, tls: VMWorkerThread) {
...
pub fn create_ug_mutator<VM: VMBinding>(
    mutator_tls: VMMutatorThread,
    mmtk: &'static MMTK<VM>,
) -> Mutator<VM> {
    let ss = mmtk.get_plan().downcast_ref::<Ungenerational<VM>>().unwrap();
    let config = MutatorConfig {
        allocator_mapping: &ALLOCATOR_MAPPING,
        space_mapping: Box::new({
            let mut vec = create_space_mapping(RESERVED_ALLOCATORS, true, ss);
            vec.push((AllocatorSelector::BumpPointer(0), &ss.copy));
            vec
        }),
        prepare_func: &common_prepare_func,
        release_func: &ug_mutator_release,
    };
