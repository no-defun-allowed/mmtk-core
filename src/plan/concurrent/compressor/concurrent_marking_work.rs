...
use crate::plan::compressor::process_edges::{PlanRemember, RemsetCondition};
...
pub struct ConcurrentTraceObjects<
    VM: VMBinding,
    P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>,
...
impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    ConcurrentTraceObjects<VM, P, C, KIND>
{
    ...
    fn scan_and_enqueue(&mut self, object: ObjectReference) {
        crate::plan::tracing::SlotIterator::<VM>::iterate_fields(
            ...
            |s| {
                ...
                if C::relevant(self.plan, s, t) {
                    self.plan.record(s, t, self.worker());
                }
...
impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    ObjectQueue for ConcurrentTraceObjects<VM, P, C, KIND>
{
...
unsafe impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    Send for ConcurrentTraceObjects<VM, P, C, KIND>
{
}

impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    GCWork<VM> for ConcurrentTraceObjects<VM, P, C, KIND>
{
...
> {
    ...
    _p: PhantomData<(VM, P, C)>,
}

unsafe impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    Send for ProcessModBufSATB<VM, P, C, KIND>
{
}

impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    ProcessModBufSATB<VM, P, C, KIND>
{
...
impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    GCWork<VM> for ProcessModBufSATB<VM, P, C, KIND>
{
    fn do_work(&mut self, worker: &mut GCWorker<VM>, mmtk: &'static MMTK<VM>) {
        let mut w = if let Some(nodes) = self.nodes.take() {
            ...
            ConcurrentTraceObjects::<VM, P, C, KIND>::new(nodes, mmtk)
        } else {
            return;
        };
...
unsafe impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    Send for ProcessRootSlots<VM, P, C, KIND>
{
}

impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    ProcessRootSlots<VM, P, C, KIND>
{
    fn create_and_schedule_concurrent_trace_objects_work(&self, objects: Vec<ObjectReference>) {
        let worker = self.worker();
        let mmtk = self.mmtk();
        let w = ConcurrentTraceObjects::<VM, P, C, KIND>::new(objects.clone(), mmtk);
...
impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    ProcessEdgesWork for ProcessRootSlots<VM, P, C, KIND>
{
    type VM = VM;
...
impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    Deref for ProcessRootSlots<VM, P, C, KIND>
{
...
impl<VM: VMBinding, P: ConcurrentPlan<VM = VM> + PlanTraceObject<VM> + PlanRemember<VM>, C: RemsetCondition<P, VM> + 'static, const KIND: TraceKind>
    DerefMut for ProcessRootSlots<VM, P, C, KIND>
{
